# PRD: PixelPal V322 — Nanobot Resource Pool (Direction B Iteration 27)

## 基本信息

| 字段 | 内容 |
|------|------|
| 提案ID | P-20260606-083 |
| 项目ID | PRJ-20260420-002 |
| 项目名 | pixel-pal-web |
| 仓库 | https://github.com/YeLuo45/pixel-pal-web |
| 开发分支 | v322-resource-pool |
| 部署分支 | gh-pages |
| 访问链接 | https://yeluo45.github.io/pixel-pal-web/ |

## 迭代目标

**Direction B iteration 27 = Resource Pool**，来源：nanobot-design。

本迭代实现资源池：资源注册、资源获取、资源释放、资源回收。

## 功能规格

### 1. 资源池架构

```
ResourceRegistrar → ResourceAcquirer → ResourceReleaser → ResourceRecycler
```

### 2. 核心模块

| 文件 | 职责 |
|------|------|
| `src/pool/ResourcePool.ts` | 资源池 |
| `src/pool/__tests__/ResourcePool.test.ts` | 测试 |

### 3. 接口设计

```typescript
interface PoolResource {
  id: string;
  type: string;
  inUse: boolean;
  acquired: number;
  released: number;
}

class ResourcePool {
  add(type: string, count: number): boolean;
  acquire(type: string): string | null;
  release(id: string): boolean;
  getStats(): { total: number; available: number; inUse: number };
}
```

## 测试要求

- 覆盖率 ≥ 99%
- 通过率 100%
- 测试位置：`src/pool/__tests__/`

## 验收标准

- [ ] `npx vitest run "src/pool/__tests__/ResourcePool.test.ts" --config vitest.config.test.ts` 全部通过
- [ ] 覆盖率报告 ≥ 99%
- [ ] `pnpm run build` 成功
- [ ] Git commit 到 `v322-resource-pool` 分支